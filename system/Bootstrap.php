...
class Bootstrap
{
    ...
    public function __construct()
    {
        ...
        if (isset($_POST['GetAllDashboardMenusForAdminOperator'])) {
            ...
            //         c.`name`
            //     FROM
            //         bnm_dashboard_menu_category c
            //     INNER JOIN bnm_dashboard_menu d ON d.category_id = c.id
            //     WHERE d.admin_operator_display = ?";
            // $cats = Db::secure_fetchall($sql, [1]);
            /////////
            $sql = "SELECT d.id did, d.fa_name dfaname, d.en_name denname,c.id cid,c.name cfaname
                FROM bnm_dashboard_menu d
                INNER JOIN bnm_dashboard_menu_category c
                ON d.category_id = c.id
                WHERE d.admin_operator_display = ? ORDER BY c.sort";
            $dm = Db::secure_fetchall($sql, [1]);
            // foreach ($dm as $k => $v) {
        ...
        if (isset($_POST['send_adminoperator_operation'])) {
            ...
            switch ($_POST['operation']) {
                ...
                case '3':
                    ...
                    // $sql="DELETE FROM bnm_users u WHERE id =:id";
                    // $res=Db::secure_update_array($sql,['id'=>$_POST['operator']]);
                    // die(json_encode($sql));
                    // Db::justexecute($sql);
        ...
        if (isset($_POST['send_administration_online_report'])) {
            ...
            if ($_POST['servicetype'] === "internet") {
                ...
                // die(json_encode($res));
                // $res['voip']="internet";
                die(Helper::Custom_Msg('در حال حاظر کاربر آنلاینی در این بخش وجود ندارد'));
            } else {
                die(Helper::Custom_Msg(Helper::Messages('f'), 2));
            }
        }
            ...
            switch ($factor[0]['sertype']) {
                ...
                case 'bitstream':
                    ...
                    foreach ($logs as $k => $v) {
                        ...
                        $v['out_bytes']             = Helper::byteConvert($v['out_bytes']);
                        $v['session_before_stop_committed_credit']         = number_format($v['session_before_stop_committed_credit']);
                        $v['duration_seconds']      = Helper::secondsToTime($v['duration_seconds']);
                ...
                case 'voip':
                    ...
                    //     $v['in_bytes']              = Helper::byteConvert($v['in_bytes']);
                    //     $v['out_bytes']             = Helper::byteConvert($v['out_bytes']);
                    //     $v['before_credit']         = number_format($v['session_before_stop_committed_credit']);
                    //     $v['duration_seconds']      = Helper::secondsToTime($v['duration_seconds']);
                    //     $v['login_time_formatted']  = Helper::tabdileTarikh($v['login_time_formatted'], 2, '-', '-', true);
                    //     $v['logout_time_formatted'] = Helper::tabdileTarikh($v['logout_time_formatted'], 2, '-', '-', true);
                    // }
                ...
                case 'sj_bs_user_telephones':
                    if (Helper::Login_Just_Check()) {
                        if (isset($_POST['condition'])) {
                            ...
                            if ($res) {
                                foreach ($res as $k => $v) {
                                    $checkprev = Helper::checkNormalPreviousFactorExist($v['sub_rowid'], $v['res_rowid'], 'bitstream');
                                ...
                                if ($res) {
                                    for ($i = 0; $i < count($res); $i++) {
                                        $checkprev = Helper::checkNormalPreviousFactorExist($res[$i]['sub_rowid'], $res[$i]['res_rowid'], 'bitstream');
                                        if ($checkprev[0]['tedad'] === 0) {
