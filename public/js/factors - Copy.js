            ...
            Factors_Initialize(selected_tab, user_id, function (data) {
                switch (selected_tab) {
                    ...
                    case 'ekhtesas_wireless_tab_link':
                        ...
                        id.val(user_id);
                        if (data) {
                            GetProvinces('popsite', function (province_res) {
                                ostan.empty();
                                if (!check_isset_message(province_res)) {
                                    for (let i = 0; i < province_res.length; i++) {
                                        ostan.append('<option value=' + province_res[i].id + '>' + province_res[i].name + '</option>');
                                    }
                                    GetCityByProvince(province_res[0]['id'], function (city_res) {
                                        shahr.empty();
                                        if (!check_isset_message(city_res)) {
                                            for (let i = 0; i < city_res.length; i++) {
                                                shahr.append('<option value=' + city_res[i].id + '>' + city_res[i].name + '</option>');
                                            }
                                            Get_Popsite_Bycity(city_res[0]['id'], function (pop_res) {
                                                popsite.empty();
                                                if (!check_isset_message(pop_res)) {
                                                    for (let i = 0; i < pop_res.length; i++) {
                                                        popsite.append('<option value=' + pop_res[i].id + '>' + pop_res[i].name + '</option>');
                                                    }
                                                    Get_Wirelessap_By_Popsite(pop_res[0]['id'], function (ap_result) {
                                                        wireless_ap.empty();
                                                        if (!check_isset_message(ap_result)) {
                                                            for (let i = 0; i < ap_result.length; i++) {
                                                                wireless_ap.append('<option value=' + ap_result[i].id + '>' + ap_result[i].name + '(' + ap_result[i].ssid + ')' + '</option>');
                                                            }
                                                            Get_Wireless_Station_By_Ap_Where_station_eshterak_null(ap_result[0]['id'], function (station_result) {
                                                                console.log('station_result');
                                                                console.log(station_result);
                                                                wireless_station.empty();
                                                                if (!check_isset_message(station_result)) {
                                                                    for (let i = 0; i < station_result.length; i++) {
                                                                        wireless_station.append('<option value=' + station_result[i].id + '>' + station_result[i].name + '</option>');
                                                                    }
                                                                } else {
                                                                    display_Predefiend_Messages(station_result);
                                                                }
                                                            });
                                                        } else {
                                                            display_Predefiend_Messages(ap_result);
                                                        }
                                                    });
                                                } else {
                                                    display_Predefiend_Messages(pop_res);
                                                }
                                            });
                                        } else {
                                            display_Predefiend_Messages(city_res);
                                        }
                                    });
                                } else {
                                    display_Predefiend_Messages(province_res);
                                }
                            });
                            //////////on change
                            ostan.on('change', function () {
                                GetCityByProvince($(this).val(), function (city_res) {
                                    shahr.empty();
                                    if (!check_isset_message(city_res)) {
                            ...
                            shahr.on('change', function () {
                                let ostan_id = ostan.find('option:selected').val();
                                let shahr_id = shahr.find('option:selected').val();
                                console.log(shahr_id);
                                if (ostan_id != null && shahr_id != null) {
                                    Get_Popsite_Bycity(shahr_id, function (pop_res) {
                                        popsite.empty();
                                        if (!check_isset_message(pop_res)) {
                                            for (let i = 0; i < pop_res.length; i++) {
                                                popsite.append('<option value=' + pop_res[i].id + '>' + pop_res[i].name + '</option>');
                            ...
                            popsite.on('change', function () {
                                let popsite_id = popsite.find('option:selected').val();
                                Get_Wirelessap_By_Popsite(popsite_id, function (ap_result) {
                                    wireless_ap.empty();
                                    if (!check_isset_message(ap_result)) {
                                        for (let i = 0; i < ap_result.length; i++) {
                                            wireless_ap.append('<option value=' + ap_result[i].id + '>' + ap_result[i].name + '(' + ap_result[i].ssid + ')' + '</option>');
                            ...
                            wireless_ap.on('change', function () {
                                let wireless_ap_id = wireless_ap.find('option:selected').val();
                                Get_Wireless_Station_By_Ap_Where_station_eshterak_null(wireless_ap_id, function (station_result) {
                                    wireless_station.empty();
                                    if (!check_isset_message(station_result)) {
                                        for (let i = 0; i < station_result.length; i++) {
                                            wireless_station.append('<option value=' + station_result[i].id + '>' + station_result[i].name + '</option>');
        ...
        Factors_Initialize('sj_bs_user_telephones', user_id, function (ports) {
            if (check_isset_message(ports)) {
                ...
                for (let i = 0; i < ports.length; i++) {
                    sj_bs_telephone.append('<option value=' + ports[i]['bor_id'] + '>' + ports[i]['telephone'] + '</option>');
                }
                                    ...
                                    sefareshe_jadid_serviceslist_li.on('click', function () {
                                        ...
                                        for (let i = 0; i < data.length; i++) {
                                            if (data[i] && data[i]['id'] === int_li_id) {
                                                ...
                                                $('#sj_bs_tarikhe_payane_service').val(data[i]['tarikhe_payane_namayesh']);
    ...
    $("#adsl_tab_link").on('click', function () {
        ...
        Factors_Initialize('sj_adsl_user_telephones', user_id, function (ports) {
            ...
            if (!check_isset_message(ports)) {
                ...
                for (let i = 0; i < ports.length; i++) {
                    if (ports[i]['telephone1']) {
                        sj_adsl_telephone.append('<option value=' + ports[i]['id'] + '>' + ports[i]['telephone1'] + '</option>');
                    }
                    if (ports[i]['telephone2']) {
                        sj_adsl_telephone.append('<option value=' + ports[i]['id'] + '>' + ports[i]['telephone2'] + '</option>');
                    }
                    if (ports[i]['telephone3']) {
                        sj_adsl_telephone.append('<option value=' + ports[i]['id'] + '>' + ports[i]['telephone3'] + '</option>');
                    }
                }
                            ...
                            sefareshe_jadid_serviceslist_li.on('click', function () {
                                ...
                                for (let i = 0; i < data.length; i++) {
                                    if (data[i] && data[i]['id'] === int_li_id) {
                                        ...
                                        //$('#sj_adsl_tarikhe_payane_service').val(data[i]['tarikhe_payane_namayesh']);
        ...
        Factors_Initialize('sj_wireless_stations', user_id, function (stations) {
            ...
            if (!check_isset_message(stations)) {
                for (let i = 0; i < stations.length; i++) {
                    if (stations[i]) {
                        sj_wireless_istgah_name.append('<option value=' + stations[i]['id'] + '>' + stations[i]['name'] + '</option>');
                    }
                }
                            ...
                            sefareshe_jadid_serviceslist_li.on('click', function () {
                                ...
                                for (let i = 0; i < data.length; i++) {
                                    if (data[i] && data[i]['id'] === int_li_id) {
                                        ...
                                        //$('#sj_wireless_tarikhe_payane_service').val(data[i]['tarikhe_payane_namayesh']);
            ...
            if (check_isset_message(data_tdlte)) {
                ...
                Get_Maliat('', function (data_maliat) {
                    Factors_Initialize('tdlte_services', false, function (data) {
                        let sj_tdlte_tdlte_number = $("#sj_tdlte_tdlte_number");
                        sj_tdlte_tdlte_number.empty();
                        for (let i = 0; i < data_tdlte.length; i++) {
                            sj_tdlte_tdlte_number.append(`<option value="${data_tdlte[i]['id']}">${data_tdlte[i]['tdlte_number']}</option>`);
                        }
                        let sefareshe_jadid_serviceslist = $("#sefareshe_jadid_serviceslist");
                            ...
                            sefareshe_jadid_serviceslist_li.on('click', function () {
                                ...
                                for (let i = 0; i < data.length; i++) {
                                    if (data[i] && data[i]['id'] === int_li_id) {
                                        ...
                                        //$('#sj_tdlte_tarikhe_payane_service').val(data[i]['tarikhe_payane_namayesh']);
                ...
                if (check_isset_message(res_print)) {
                    ...
                    switch (res_print[0]['type']) {
                        ...
                        case 'bitstream':
                            ...
                            pa_address.text('آدرس : ' + res_print[0]['address1']);
                            ...
                            pa_noe_khadamat.text(res_print[0]['noe_khadamat']);
                        ...
                        case 'tdlte':
                            ...
                            wa_address.text('آدرس : ' + res_print[0]['address1']);
                            ...
                            wa_noe_khadamat.text(res_print[0]['noe_khadamat']);
                        ...
                        case 'voip':
                            ...
                            vo_address.text('آدرس : ' + res_print[0]['address1']);
                            vo_name_va_family.text('نام مشترک / (شرکت) :' + res_print[0]['name'] + ' ' + res_print[0]['f_name']);
                            vo_tozihate_factor.text(res_print[0]['tozihate_faktor']);
                            vo_noe_khadamat.text(res_print[0]['noe_khadamat']);
