...
class SiamHelper
{
    ...
    public static function MacSearch($username, $password, $macaddr, $fdate, $tdate, $test=false)
    {
        ...
        $rfdate=Helper::regulateNumber($fdate,1);
        $rtdate=Helper::regulateNumber($tdate,1);
        $dashed_enfdate = Helper::fixDateDigit(Helper::TabdileTarikh($rfdate, 2, '/', '-', true), '-');
        $dashed_entdate = Helper::fixDateDigit(Helper::TabdileTarikh($rtdate, 2, '/', '-', true), '-');
        $slashed_enfdate= Helper::fixDateDigit(Helper::TabdileTarikh($rfdate, 2, '/', '/', true), '/');
        $slashed_entdate= Helper::fixDateDigit(Helper::TabdileTarikh($rtdate, 2, '/', '/', true), '/');
        // $en_fdate       = Helper::TabdileTarikh($en_fdate, 2, '/', '/', false);
        // $en_tdate       = Helper::TabdileTarikh($en_tdate, 2, '/', '/', false);
        // $en_fdate       = Helper::fixDateDigit($en_fdate, '/');
        // $en_tdate       = Helper::fixDateDigit($en_tdate, '/');
            ...
            for ($i = 0; $i < count($onlineusersinfo); $i++) {
                if (isset($onlineusersinfo[$i]['internet_onlines'])) {
                    if (($onlineusersinfo[$i]['internet_onlines'])) {
                        if (isset($onlineusersinfo[$i]['internet_onlines'][0])) {
                            if (($onlineusersinfo[$i]['internet_onlines'][0])) {
                                if ($onlineusersinfo[$i]['internet_onlines'][0][7] && $onlineusersinfo[$i]['internet_onlines'][0][7] !== '-') {
                                    if ($onlineusersinfo[$i]['internet_onlines'][0][7] === $macaddr) {
                                        $isonline = true;
                                        $onlineinfo['ipv4_address'] = $onlineusersinfo[$i]['internet_onlines'][0][4];
                                        $onlineinfo['mac'] = $onlineusersinfo[$i]['internet_onlines'][0][7];
                                        $onlineinfo['session_start_time'] = $onlineusersinfo[$i]['basic_info']['last_login'];
                                        ...
                                        //////adding online info to logs
                                        // if($logs){
                                        //     $logs=array_merge([$online_log], $logs);
                                        // }else{
                                        //     $logs=[$online_log];
                                        // }
                                        // $logs = self::unsetExtraArrayElems($logs, __SIAMLIMITATION__);
        ...
        $logs = $GLOBALS['ibs_internet']->getConnectionsByMacAndDateTime($macaddr, $rfdate, $rtdate, 100000, 'jalali');
        // $logs=$GLOBALS['ibs_internet']->getConnectionsByMacAndDateTime($macaddr, Helper::regulateNumber($fdate), Helper::regulateNumber($tdate), 100000, 'jalali');
        if (Helper::checkIbsResultStrict($logs)) {
            $logs = $logs[1][1];            
            $logs = self::unsetLogIfIPNotValid($logs);
            $logs = self::unsetLogOutOfSessionStartStop($logs, $dashed_enfdate, $dashed_entdate, 'g', 'd');
            ...
            if(count($logs)< __SIAMLIMITATION__){
                // $log2=$GLOBALS['ibs_internet']->getConnectionByRemoteIpAndDateTimeDesc($regulatedip, false, $logs[array_key_last($logs)]['session_start_time'], 20000);
                $log2 = $GLOBALS['ibs_internet']->getConnectionsByMacAndDateTime($macaddr, false, $logs[array_key_last($logs)]['session_start_time'], 10000);
                if (Helper::checkIbsResultStrict($log2)) {
                        $log2 = $log2[1][1];
                        $log2 = self::unsetLogIfIPNotValid($log2);
                        if($log2){
                            $log2=self::findLogBeforeAndAfterDatetime($log2, $slashed_enfdate);
                            if($log2){
                                $logs=array_merge($logs, [$log2]);
                            }
                        }
        ...
        //     $logs=[];
        // }
        if($isonline && $onlineinfo){
            $logs=array_merge([$onlineinfo], $logs);
        }
        if(! $logs) return json_encode('QryResult=0', JSON_UNESCAPED_UNICODE); 
        $logs = self::unsetExtraArrayElems($logs, __SIAMLIMITATION__);
        //search with mac has bug so i decided to search with username
        foreach ($logs as $k => $v) {
            if(! isset($v['username'])){
                unset($logs[$k]);
            }else{
        ...
        $usernamelogs=Helper::getIbsLogsByArrayUsernamesDesc($usernames, $rfdate, $rtdate, 20000, 'jalali');
        if (Helper::checkIbsResultStrict($usernamelogs)) {
            $usernamelogs=$usernamelogs[1][1];
            // $logs=$usernamelogs;
            if($onlineinfo){
                $logs=array_merge([$onlineinfo], $usernamelogs);
            }else{
                $logs=$usernamelogs;
            }
            $logs = self::unsetLogIfIPNotValid($logs);
    ...
    public static function ApplySuspIp($username, $password, $RefNum, $SuspId, $SuspType, $SuspOrder, $tel = '', $ip = '', $cid = '')
    {
        ...
        //     $SuspType=0;
        // }
        // return [$SuspId, $SuspType, $SuspOrder];
        if((int) $SuspId===0){
            if($SuspType!==0){
                Helper::logLockInternetService($serinfo['username'], 2, $SuspType, $SuspOrder, $RefNum, 'قطع از طریق سیام');
            }
        }else{
            Helper::logUnlockInternetService($serinfo['username'], 2, $SuspType, $SuspOrder, $RefNum, 'وصل از طریق سیام');
        }
//درخواست با موفقیت انجام گردید در حال حاظر دستور قطع دیگری نیز بر روی این سرویس فعال میباشد.


        if((int) $SuspId===0){
            if((int)$SuspType===0){
                if(Helper::checkUserCanBeUnsuspended($serinfo['username'], 2) && Helper::checkUserCanBeUnsuspended($serinfo['username'], 1)){
                    $res=Helper::lockService($serinfo['username']);
                    $res=$GLOBALS['ibs_internet']->killUser($ibsid);
                    sleep(5);
            ...
            if(Helper::checkUserCanBeUnsuspended($serinfo['username'], 2) && Helper::checkUserCanBeUnsuspended($serinfo['username'], 1)){
                ...
                if(isset($info['attrs']['lock'])){
                    ...
                    return json_encode('QryResult=2' . self::sep('f') . "این تلفن در حال حاظر وصل میباشد.", JSON_UNESCAPED_UNICODE);
                }
            }else{
                return json_encode('QryResult=0' . self::sep('f') . ".درخواست با موفقیت انجام گردید", JSON_UNESCAPED_UNICODE);
            }
        ...
        //     if((int)$SuspId===0){
        //         if((int) $SuspType===0){
        //             if(Helper::checkUserCanBeUnsuspended($serinfo['username'], 1) && Helper::checkUserCanBeUnsuspended($serinfo['username'], 2)){
        //                 $res=Helper::lockService($serinfo['username']);
        //                 $res=$GLOBALS['ibs_internet']->killUser($ibsid);
        //                 sleep(5);
        //                 $unlock=Helper::unlockService($serinfo['username']);
        ...
        //             }
        //         }else{
        //             // $log=Helper::logLockInternetService($serinfo['username'], $SuspType, $SuspOrder, $RefNum, 'قطع بدستور سیام', 2);
        //             $res=Helper::lockService($serinfo['username']);
        //             $res=$GLOBALS['ibs_internet']->killUser($ibsid);
        //             return json_encode('QryResult=2' . self::sep('f') . "این تلفن در حال حاضر قطع میباشد. درخواست قطع شما نیز روی آن اعمال شد", JSON_UNESCAPED_UNICODE);
        //         }
        //     }else{
        //         //vasl
        //         // $log=Helper::logUnlockInternetService($serinfo['username'], $SuspType, $SuspOrder, $RefNum, 'وصل بدستور سیام', 2);
        //         if(Helper::checkUserCanBeUnsuspended($serinfo['username'], 1) && Helper::checkUserCanBeUnsuspended($serinfo['username'], 2)){
        //             $res=Helper::unlockService($serinfo['username']);
        //             return json_encode('QryResult=0' . self::sep('f') . ".درخواست با موفقیت انجام گردید", JSON_UNESCAPED_UNICODE);
        //         }else{
        //             return json_encode('QryResult=2' . self::sep('f') . "این تلفن در حال حاضر قطع میباشد. درخواست وصل شما نیز روی آن اعمال شد", JSON_UNESCAPED_UNICODE);
        //         }
        //     }
        // }else{
        //     if((int)$SuspId===0){
