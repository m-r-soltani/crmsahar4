...
class JsonRpc2_old
{
	...
	public function __mycall($method, array $params)
    {
        if ($this->isNamedArguments && count($params) === 1 && is_array($params[0])) {
            $params = $params[0];
        }

        return $this->myexecute($method, $params);
	}
	public function myexecute($procedure, array $params = array(), array $reqattrs = array(), $requestId = null, array $headers = array())
    {
    ...
    public function __call($pMethod, array $pParams)
    {
		...
		// return $resultStatus = curl_getinfo($ch, CURLINFO_HTTP_CODE);
        if (curl_errno($ch)) {
