...
class IBSjsonrpcClient {
    ...
    */
    ...
    function __construct($server_ip, $auth_name, $auth_pass, $auth_type="ADMIN", $server_port=1237, $timeout=5000){
    ...
    // public static function ibsConnect($server_ip="80.253.151.5", $auth_name="system", $auth_pass="si@h@sti1355", $auth_type="ADMIN", $server_port=1237, $timeout=5000){
    //     self::$instance = new jsonrpc_client($server_ip. ':' . $server_port);
    ...
    function getUserInfoByUserID($user_id, $onlyResultArrayReturn=true){
        $res= $this->sendRequest("user.getUserInfo", array("user_id"=>(string)$user_id));
        if(! isset($res[0])){
            return false;
        }
        if(! ($res[0])){
    ...
    function getUserInfoByVoipUserName($normal_username){
        return $this->sendRequest("user.getUserInfo", array("voip_username"=>$normal_username));
    }
    ...
    function unlockUser($user_id){
        return $this->deleteUserAttribute($user_id, "lock");
    }
    ...
    function deleteRelExpDate($user_id){
        return $this->deleteUserAttribute($user_id, array("rel_exp_date"));
    }
    ...
    function deleteAbsExpDate($user_id){
        return $this->deleteUserAttribute($user_id, array("abs_exp_date"));
    }
    ...
    function searchUser($conds){
        $res = $this->sendRequest("user.searchUser", array(
            "conds" => $conds,
            "from" => 1,
            "to" => 50000,
            "order_by" => "user_id",
    ...
    function findOnlineUsers($returnIdsOnly=true){
        $res = $this->sendRequest("user.searchUser", array(
            "conds" => ['is_online'=>TRUE],
            "from" => 1,
            "to" => 50000,
            "order_by" => "user_id",
            "desc" => FALSE
    ...
    function getAllVoipOnlineUsers(){
        $result = $this->sendRequest("report.getOnlineUsers", array(
            "conds" => array(),
            "voip_sort_by" => "user_id",
            "voip_desc" => FALSE,
    ...
    function listISPsWithIDs(){
        $result = $this->sendRequest("report.listISPsWithIDs", array(
            "conds" => [],
            "from" => 1,
            "to" => 50000,
    ...
    function getConnectionsByUserIdAndDateTimeASC($userid, $login_time_from, $login_time_to, $date_unit="gregorian"){
        $conds = array(
            "user_ids" => (string)$userid,
            "login_time_from" => $login_time_from,
            "login_time_from_unit" => $date_unit,
            "logout_time_to" => $login_time_to,
            "logout_time_to_unit" => $date_unit,
        );
        $result = $this->sendRequest("report.getConnections", array(
            "conds" => $conds,
            "from" => 1,
            "to" => 50000,
            "sort_by" => "session_stop_time", ## FIXME
            "desc" => FALSE
        ));
    ...
    function getConnectionsByMacAndDateTime($mac, $fdate, $tdate, $rows=200, $date_unit="gregorian"){
        if(! $fdate){
            $fdate=date("Y-m-d", strtotime("-6 months"))." ".Helper::nowTimeTehran();
        }
        if(! $tdate){
            $tdate=date("Y-m-d")." ".Helper::nowTimeTehran();
        }
        $conds = array(
            "mac" => $mac,
            "login_time_from" => $fdate,
            "login_time_from_unit" => $date_unit,
            "logout_time_to" => $tdate,
            "logout_time_to_unit" => $date_unit,
        );
        $result = $this->sendRequest("report.getConnections", array(
            "conds" => $conds,
    ...
    function getConnectionByRemoteIpDesc($ip, $rows=200){
        $conds = array(
            "remote_ip" => $ip,
        );
        $result = $this->sendRequest("report.getConnections", array(
    ...
    function getConnectionByUserIdDesc($userids, $rows=200){
        $conds = array(
            "user_ids" => $userids,
        );
        $result = $this->sendRequest("report.getConnections", array(
    ...
    function getConnectionByUserIdAndDateTimeDesc($userids, $fdate=false, $tdate=false, $rows=200, $date_unit="gregorian"){
        if(! $fdate){
    ...
    function getConnectionByUserIdAndUserIdAndDateTimeDesc($userids, $ip, $fdate=false, $tdate=false, $rows=200, $date_unit="gregorian"){
        if(! $fdate){
            $fdate=date("Y-m-d H:i:s", strtotime("-6 months"));
        }
        if(! $tdate){
    ...
    function getConnectionByRemoteIpAndDateTimeDesc($ip, $fdate=false, $tdate=false, $rows=200, $date_unit="gregorian"){
        if(! $fdate){
            $fdate=date("Y-m-d H:i:s", strtotime("-6 months"));
        }
        if(! $tdate){
    ...
    function getConnectionByUsernameAndDateTimeDesc($username, $fdate= false, $tdate= false, $rows=201, $date_unit="gregorian"){
    ...
    function getConnectionByUsernameAndRemoteIpDesc($username, $ip, $fdate=false, $tdate=false, $rows=201, $date_unit= "gregorian"){
        if(! $fdate){
            $fdate=date("Y-m-d H:i:s", strtotime("-6 months"));
        }
        if(! $tdate){
    ...
    function getConnectionLogsVoip($login_time_from, $login_time_to, $user_ids=null, $remote_ip=null, $date_unit="gregorian"){
        $conds = array(
            "login_time_from" => $login_time_from,
            "login_time_from_unit" => $date_unit,
        ...
        foreach($report as $item){
            ...
            $report_abs[] = array(
                ...
                "login_time" => $item["login_time_formatted"],
                "logout_time" => $item["logout_time_formatted"],
                // "voip_in_bytes" => $item["voip_in_bytes"],
                // "voip_out_bytes" => $item["voip_out_bytes"],
                // "remote_ip" => $item["remote_ip"],
                "mac" => $mac
            );
        }
    ...
    function getUserIdByVoipUsername($username){
        $res = $this->getUserInfoByVoipUserName($username);
        if($res[0]){
            if($res[1]){
                if(key($res[1])){
                    return key($res[1]);
    ...
    function getUsersAboutToExpire(int $d1=3,int $d2=3){
        return $this->sendRequest("user.searchExpiredUsers", array(
            "conds" => array(
                "exp_date_from" => (string)$d1,
                "exp_date_from_unit" => "days",
                "exp_date_to" => "-".(string)$d2,
                "exp_date_to_unit" => "days",
            ),
            "from" => 0,
            "to" => 100000,
    ...
    function getTwoMonthExpiredUsers(){
        return $this->sendRequest("user.searchExpiredUsers", array(
            "conds" => array(
                "exp_date_from" => "61",
                "exp_date_from_unit" => "days",
