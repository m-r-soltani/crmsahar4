...
// Https://IpAddress:port/..../DServices/....?WSDL
    ...
    public static function makeQueryResult($res){
        $qr="QryResult=";
    ...
    public static function combinSearch($username, $password, $tel = '', $ip = '', $name = '', $family = '', $ss = '', $code_posti = '', $code_meli = '', $passno = '')
    {
        ...
        if($subs && $regulatedip){
            ...
            // return $usernames;
            if(! $usernames) return json_encode('QryResult=0', JSON_UNESCAPED_UNICODE);
            ...
            if(! $usernames) return json_encode('QryResult=0', JSON_UNESCAPED_UNICODE);
            $logs=Helper::getIbsLogsByArrayUsernamesDesc($usernames, false, false, 10000);
            if(! Helper::checkIbsResultStrict($logs)){
                return json_encode('QryResult=0', JSON_UNESCAPED_UNICODE);
            }
            $logs=$logs[1][1];
        ...
        for ($l=0; $l <count($logs) ; $l++) { 
            ...
            if(! isset($logs[$l]['subinfo'])) unset($logs[$l]);
            if(! $logs[$l]['subinfo']) unset($logs[$l]);
            if(! isset($logs[$l]['serinfo'])) unset($logs[$l]);
            if(! $logs[$l]['serinfo']) unset($logs[$l]);
        }
        ...
        if(! isset($ressub)) return json_encode('QryResult=5'.self::sep('f')."خطای پایگاه داده", JSON_UNESCAPED_UNICODE);
        for ($l=0; $l <count($logs) ; $l++) {
        ...
        //everything is done now we can make result and send it
        $logs=array_values($logs);
        $res=[];
        $z=0;
        for ($i=0; $i <count($logs) ; $i++) {
            if(! isset($logs[$i]['subinfo'])) continue;
            if(! ($logs[$i]['subinfo'])) continue;
            if(! isset($logs[$i]['serinfo'])) continue;
            if(! ($logs[$i]['serinfo'])) continue;
            ...
            $res[$z][]= Helper::regulateNumber(Helper::fixDateDigit((string) Helper::TabdileTarikh($logs[$i]['serinfo']['siam_tarikhe_tasvie_shode'], 1, '-', '/', false)), 2) ;
            ...
            $res[$z][]= Helper::regulateNumber(Helper::fixDateDigit((string) Helper::TabdileTarikh($logs[$i]['subinfo']['tarikhe_tavalod'], 1, '-', '/', false)), 2) ;
    ...
    public static function TechnicalSearch($username, $password, $ip, $fdate, $tdate){
                        ...
                        if(isset($onlineusersinfo[$i]['internet_onlines'][0])){
                            if(($onlineusersinfo[$i]['internet_onlines'][0])){
                                if($onlineusersinfo[$i]['internet_onlines'][0][4] && $onlineusersinfo[$i]['internet_onlines'][0][4] !== '-' && $onlineusersinfo[$i]['internet_onlines'][0][4]!== '0.0.0.0'){
                                    if($onlineusersinfo[$i]['internet_onlines'][0][4] === $regulatedip){
                                        $isonline                       = true;
                                        ...
                                        if(! isset($onlineinfo['serinfo'])){
                                            return json_encode('QryResult=0', JSON_UNESCAPED_UNICODE);
                                        }
                                        if(! ($onlineinfo['serinfo'])){
                                            return json_encode('QryResult=0', JSON_UNESCAPED_UNICODE);
                                        }
                                        $subinfo=self::selectMultiSubsByIds([$onlineinfo['serinfo']['subid']]);
                                        $onlineinfo['subinfo']=$subinfo;
                                        //find las connection log of user
                                        $prevlog=Helper::getIbsLogsByArrayUsernamesDesc([$onlineinfo['username']], date("Y-m-d H:i", strtotime("-6 month")), $onlineinfo['session_start_time'], 10000);
                                        if(! Helper::checkIbsResultStrict($prevlog)){
                                            ...
                                            if(! $prevlog){
                                                $onlineinfo['lastlogout']='';
                                            }else{
                                                $onlineinfo['lastlogout']=$prevlog[0]['ipv4_address'];
                                            }
                                        }
                                        break;
                                    }
        ...
        // return $onlineinfo;
        ////////connection log result
        $logs=$GLOBALS['ibs_internet']->getConnectionByRemoteIpAndDateTimeDesc($regulatedip, $dashed_fdate, $dashed_tdate, 10000, "jalali");
        ////combining online info + logs
        if(Helper::checkIbsResultStrict($logs) && $isonline){
            //is online & has log
            $logs=$logs[1][1];
            ...
            $logs=self::unsetExtraArrayElems($logs, __SIAMLIMITATION__);
        }elseif(Helper::checkIbsResultStrict($logs) && ! $isonline){
            $logs=$logs[1][1];
            //not online & has log
            $logs=self::unsetLogIfIPNotValid($logs, "ipv4_address");
            $logs=self::unsetExtraArrayElems($logs, __SIAMLIMITATION__);
                ...
                if($logs[$i]){
                    if(isset($logs[$i]['subinfo']) && isset($logs[$i]['serinfo'])){
                        if($logs[$i]['subinfo'] && $logs[$i]['serinfo']){
                            ...
                            $res[$z][]= $logs[$i]['subinfo']['f_name'] ;
                            $res[$z][]= $logs[$i]['subinfo']['name_pedar'] ;
                            $res[$z][]= Helper::regulateNumber($logs[$i]['subinfo']['shomare_shenasname'],2) ;
                            ...
                            $res[$z][]= $logs[$i]['subinfo']['fulladdress'] ;
                            $res[$z][]= Helper::regulateNumber((string) Helper::TabdileTarikh($logs[$i]['subinfo']['siam_tarikhe_sabtenam'], 1, '-', '/', false),2);
                            ...
                            if($logs[$i]['subinfo']['noe_moshtarak']==="real"){
                                ...
                                $res[$z][]= $logs[$i]['subinfo']['name_sherkat'];
                                $res[$z][]= $logs[$i]['subinfo']['shomare_sabt'];
                                $res[$z][]= $logs[$i]['subinfo']['code_eghtesadi'];
                            }
                            $res[$z][]= $logs[$i]['subinfo']['ostan_faname'];//name ostane vagozar konande 
                            $res[$z][]= $logs[$i]['subinfo']['shahr_faname'];//name ostane vagozar konande 
                            ...
                            $res[$z][]=$logs[$i]['username'];
                            $ibsinfo=$GLOBALS['ibs_internet']->getUserInfoByUserID($logs[$i]['user_id']);
                            ...
                            if(isset($logs[$i]['remaining_credit'])){
                                $res[$z][]=(string)$logs[$i]['remaining_credit'];
                            }else{
                                $res[$z][]='';
                            }
                            ...
                            if(isset($logs[$i+1])){
                                if($logs[$i]['username']===$logs[$i+1]['username']){
                                    if($logs[$i+1]['session_stop_time']){
                                        // $res[$z][]= Helper::TabdileTarikh($logs[$i+1]['session_stop_time'], 1, '-', '/');
                                        // $res[$z][]= Helper::regulateNumber(Helper::TabdileTarikh(date("Y-m-d H:i",strtotime($logs[$i+1]['session_stop_time'])), 1, '-', '/', true), 2);//check kon
                                        $prevsesstop=date('Y-m-d H:i', strtotime($logs[$i+1]['session_stop_time']));
                                    }else{
                                        $prevsesstop='';
                                    }
                                }else{
                                    /////get data from log ibs
                                    $prevlog=$GLOBALS['ibs_internet']->getConnectionByUsernameAndDateTimeDesc($logs[$i]['username'], false, $logs[$i]['session_start_time'], 2000);
                                    if(Helper::checkIbsResultStrict($prevlog)){
                                        $prevlog=$prevlog[1][1];
                                        $prevlog=self::unsetLogIfIPNotValid($prevlog);
                                        if($prevlog){
                                ...
                                ///there is no prev log in current logs
                                $prevlog=$GLOBALS['ibs_internet']->getConnectionByUsernameAndDateTimeDesc($logs[$i]['username'], false, $logs[$i]['session_start_time'], 2000);
                                if(Helper::checkIbsResultStrict($prevlog)){
                                    $prevlog=$prevlog[1][1];
                                    $prevlog=self::unsetLogIfIPNotValid($prevlog);
    ...
    public static function TechnicalSearch_log_and_onlineuser_before_change($username, $password, $ip, $fdate, $tdate){
        $auth=self::authentication($username, $password);
        if(! $auth) return json_encode('QryResult=3'.self::sep('f')."نام کاربری یا رمز عبور اشتباه است", JSON_UNESCAPED_UNICODE);
        $regulatedip=Helper::regulateNumber($ip, 1);
        $checkipexists=Helper::checkIpExist(Helper::regulateNumber($regulatedip, 1));
                        ...
                        if(isset($onlineusersinfo[$i]['internet_onlines'][0])){
                            if(($onlineusersinfo[$i]['internet_onlines'][0])){
                                if($onlineusersinfo[$i]['internet_onlines'][0][4] && $onlineusersinfo[$i]['internet_onlines'][0][4] !== '-' && $onlineusersinfo[$i]['internet_onlines'][0][4]!== '0.0.0.0'){
                                    if($onlineusersinfo[$i]['internet_onlines'][0][4] === $regulatedip){
                                        $isonline               = true;
                                        $onlineinfo['ip']       = $onlineusersinfo[$i]['internet_onlines'][0][4];
                                        $onlineinfo['mac']      = $onlineusersinfo[$i]['internet_onlines'][0][7];
                                        $onlineinfo['logintime']= $onlineusersinfo[$i]['basic_info']['last_login'];
                                        $onlineinfo['credit']   = $onlineusersinfo[$i]['basic_info']['credit'];
                                        $onlineinfo['username'] = $onlineusersinfo[$i]['attrs']['normal_username'];
                                        $onlineinfo['password'] = $onlineusersinfo[$i]['attrs']['normal_password'];
                                        $onlineinfo['user_id']  = $onlineusersinfo[$i]['user_id'];
                                    }
        ...
        if($isonline){
            ...
            for ($i=0; $i <count($allservices) ; $i++) { 
                if($allservices[$i]['ibsusername']){
                    if(Helper::like($allservices[$i]['ibsusername'], $onlineinfo['username'])){
                        // $allservices=$allservices[$i];
                        $onlineserinfo=$allservices[$i];
                        break;
                    }
                }
            }
            if(! ($onlineserinfo)) return json_encode('QryResult=0', JSON_UNESCAPED_UNICODE);//todo... other error
            if(! isset($onlineserinfo['ibsusername'])) return json_encode('QryResult=0', JSON_UNESCAPED_UNICODE);//todo... other error
            if(! ($onlineserinfo['ibsusername'])) return json_encode('QryResult=0', JSON_UNESCAPED_UNICODE);//todo... other error
            $subinfo=self::selectMultiSubsByIds([$onlineserinfo['subid']]);
            //find las connection log of user
            $prevlog=Helper::getIbsLogsByArrayUsernamesDesc([$onlineinfo['username']], date("Y-m-d H:i", strtotime("-6 month")), $onlineinfo['logintime'], 2000);
            if(! Helper::checkIbsResultStrict($prevlog)){
                $prevlog=false;
            }else{
                $prevlog=$prevlog[1][1];
        ...
        return 'asd';
        ////////connection log result
        $logs=$GLOBALS['ibs_internet']->getConnectionByRemoteIpAndDateTimeDesc($regulatedip, $dashed_fdate, $dashed_tdate, 2000, "jalali");
        if(! Helper::checkIbsResultStrict($logs)){
            return json_encode('QryResult=0', JSON_UNESCAPED_UNICODE);
        }
        $logs=$logs[1][1];
        ...
        for ($i=0; $i <count($logs) ; $i++) {
                ...
                $res[$z][]=$logs[$i]['username'];
                $ibsinfo=$GLOBALS['ibs_internet']->getUserInfoByUserID($logs[$i]['user_id']);
                if(isset($ibsinfo[1][key($ibsinfo[1])])){
                    $ibsinfo=$ibsinfo[1][key($ibsinfo[1])];
                }
                if (isset($ibsinfo['attrs']['normal_password'])) {
                        $res[$z][]= $ibsinfo['attrs']['normal_password'];
                        // $res .= str_replace('.', ',', Helper::byteConvert((int) Helper::toByteSize((string) $ibsinfo['basic_info']['credit'] . 'MB')));
    ...
    public static function TechnicalSearch_notacceptable($username, $password, $ip, $fdate, $tdate){
        $auth=self::authentication($username, $password);
        if(! $auth) return json_encode('QryResult=3'.self::sep('f')."نام کاربری یا رمز عبور اشتباه است", JSON_UNESCAPED_UNICODE);
        $regulatedip=Helper::regulateNumber($ip, 1);
        $checkipexists=Helper::checkIpExist(Helper::regulateNumber($regulatedip, 1));
    ...
    public static function MacSearch($username, $password, $macaddr, $fdate, $tdate){
        ...
        $ibsresult=$GLOBALS['ibs_internet']->getConnectionsByMacAndDateTimeASC($macaddr, $fdate, $tdate, 200, "jalali");
        // return $ibsresult;
        if(! Helper::checkIbsResultStrict($ibsresult)){
            return json_encode('QryResult=0', JSON_UNESCAPED_UNICODE);
        }
        $logs=$ibsresult[1][1];
        $services=Helper::getAllInternetUsersServicesInfoNoAuth();
        $qr='QryResult=';
        $res=[];
        $z=0;
        $logs=array_values($logs);
        for ($i=0; $i <count($logs) ; $i++) { 
            ...
            $res[$z][]=Helper::regulateNumber($fa_fdate, 2);
            ////stop datetime
            $end_time=date("Y-m-d h:i", strtotime($logs[$i]['session_stop_time']));
            $fa_tdate=Helper::TabdileTarikh($end_time, 1, '-', '/', true);
            $res[$z][]=Helper::regulateNumber($fa_tdate, 2);
            for ($s=0; $s <count($services) ; $s++) { 
                if(Helper::like($logs[$i]['username'], $services[$s]['ibsusername'])){
                    //telephone1
                    $res[$z][]= self::siamTel($services[$s]['telephone1'], 2);
                    /////////ip
                    if($logs[$i]['ipv4_address'] && $logs[$i]['ipv4_address']!=='-'){
                        $res[$z][]= Helper::regulateNumber($logs[$i]['ipv4_address'], 2);
                    }else{
                        $res[$z][]='';
                    }
                    ///sertype
                    $res[$z][]= $services[$s]['general_sertype'];
                    break;
                }
            }
            $z++;
